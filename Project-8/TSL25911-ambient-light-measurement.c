/**
 * 
 * TSL25911 ambient light sensor test code as of March 14, 2026
 * 
...
void OLED_showDigits(uint digit, uint a, uint b, uint c , uint d)
{
...
void TSL25911_ReadWriteI2C(uint8_t command[], uint8_t length){
	
	i2c_write_blocking(I2C_ID, 0x29, command, length, false); // this device obviously doesn't care about Start and Stop Bit, so "true" is also fine (i. e. the final stop bit is not sent, data transfer continues as part of the same transaction)
    while (i2c_read_blocking(I2C_ID, 0x29, command, length, false) < 0)  // repeats until the slave device sends ACK
    {
      sleep_ms(1);
    };  	
}
...
{  
    ...
    */
    ...
    while(true){ 
      ...
      i2c_write_blocking(I2C_ID, 0x29, TSL25911_STATUS, 1, false);    
      i2c_read_blocking(I2C_ID, 0x29, TSL25911_STATUS, 1, false);   
      ...
      uart_write_blocking(UART_ID, readValue, strlen(readValue));
      sprintf(readValue, "TSL25911 Status Register: %x \r\n", TSL25911_STATUS[0]);  // values from 0 to 255
      uart_write_blocking(UART_ID, readValue, strlen(readValue));       
