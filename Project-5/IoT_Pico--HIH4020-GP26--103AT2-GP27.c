...
{    
    ...
    {   
        ...
        float readRHSensor = ((((adc_read()-815)/2.6)*3.3)/4095)*100; // maths for the RH sensor HIH4020 (including an offset); voltage divider with 10k+10k and 100k on sensor output; further explanation see data sheet       
        ...
        uint16_t readAmbientLight = adc_read(); // raw value (max 4095); I use one SFH 300 phototransistor as sensor and two 10k resistors as voltage dividers (supply voltage VSYS 5V)

        char data[2048];
        
        sprintf(data, "HTTP/1.0 200 OK\r\nServer: Pico\r\nContent-type: text/html\r\n\r\n<html><head><title>Weather</title></head><body><p><font color=blue>Relative Humidity (0-100): %u percent</font>; <font color=red>Ambient Temperature: %2.1f degrees Celsius</font>; <font color=Orange>Ambient Light (12bit raw): %u</font>; </p><p><font color=brown>Time since StartUp: %u minutes (%2.2f hours)</p><p>WD indicator %u</font></p><p>SW V1.3; (c) ome-eng.net 2023</p></body></html>\r\n", outputRHSensor, readTempSensor, readAmbientLight, absoluteTimeMinutes, absoluteTimeHours, wd_indicator);
