 ...
 * 
 * THERMAL IMAGING
 * Test code as of September 21, 2025
 * 
 * SW-V 0.2x -> OLED display (this program)
 * SW-V 0.4x -> IoT 
 * 
 * MLX90642 IR array serves as main sensor
 * TSL25911 ambient light sensor is an additional component to stress the I2C a bit 
 * 
 * OLED for thermal display, some data output on the serial port
 *  
 */
...
void initI2C()
{  
  i2c_init(I2C_ID, 400 * 1000);    // as to the MLX90642, 1 MHz would theoretically be possible
...
void renderImage(uint digit, uint a, uint b, uint c , uint d)
{
    ...
    if (digit == 0)
        ...
        render(quarter_full, &frame_area);
    else if (digit == 3)
    ...
    */ 
    ...
    sleep_ms(1);
     ...
     // test I2C commands for TSL25911
      ...
      float decimalValueTemperature;
      
      uint16_t pixelAddress;
      
      float threshold_1 = 32.0;
      float threshold_2 = 28.0;
      float threshold_3 = 27.0;
      ...
      while(true){   
		  ...
		  for (int m = 0; m <= 3; m++)
		  {
            ...
            { 
             ...
             // there are open questions as to using addresses correctly: which RAM address is which pixel exactly?
             if (m == 0)
               pixelAddress = 0x342C;
             else if (m == 1)
               pixelAddress = 0x3462;
             ...
             decimalValueTemperature = ((pixelValue[0] << 8) + pixelValue[1]) / 50.0;  // see data sheet for the formula     
           
             sleep_ms(2);
                      
              if (decimalValueTemperature > threshold_1)
                renderImage(0, (0+i*8), (7+i*8), m, m); 
              else if ((decimalValueTemperature > threshold_2) && (decimalValueTemperature <= threshold_1))
                renderImage(1, (0+i*8), (7+i*8), m, m);
              else if ((decimalValueTemperature > threshold_3) && (decimalValueTemperature <= threshold_2))
                renderImage(2, (0+i*8), (7+i*8), m, m); 
              else if (decimalValueTemperature <= threshold_3)
                renderImage(3, (0+i*8), (7+i*8), m, m); 
