...
# threshold above arbitrary; resistive type sensor delivers reliably 0V as output when the probe is dry (HW-390 >2.0V)
    ...
    if OLED_true == 1:
      ...
      oled.text("type: Resistive",0,40)      
      oled.show()
else:
    resistivTrue=0
    if OLED_true == 1:
      oled.text("Detected sensor",0,30)
      oled.text("type: Capacitive",0,40)
      oled.show()
...
while True:
    ...
    if sample_hold>0.20: # new read
       ...
       if resistivTrue==0:
          # mathematical optimization to utilize full 0-99 range on the 7-Segment-Displays (can never be perfect because there always exists variance in individual sensor);
