/**
 * Created 02/08/2023 by Florian Wilhelm Dirnberger
 *
...
void measure_freqs(void) {
    ...
    */
    ...
    itoa(f_rosc, snum, 10); // will have slightly different value after every power up
    lcd_set_cursor(1,0);
    lcd_string(snum);
    
    itoa(f_clk_sys, snum, 10); // must be 12MHz
    lcd_set_cursor(1,9);
    lcd_string(snum);
    
    itoa(f_pll_sys, snum, 10); // must be 125MHz
    lcd_set_cursor(1,13);
    lcd_string(snum);
...
int main() {
    ...
    *(XOSC + 0x00 / 4) = 0x00fabaa0;  // Crystal Oscillator can be de-activated by using hex value 0x00d1eaa0  
