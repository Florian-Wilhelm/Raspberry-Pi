...
void writeLocalCharacteristicBLE(uint32_t cpm)
{
    ...
    sleep_ms(200); 
    * uart_write_blocking(UART_ID, SendData6, strlen(SendData6));    
    sleep_ms(200);   
    */
    ...
    */
    ...
    for (;;) {
       ...
       if (((absolutZeit)/(timeFrame*n)) == 1) { 
         ...
         cpm = (pulsesTimeframe/timeFrame)*60.0f;  // Extrapolation
         pulsesTimeframe = 0;                            
         
         renderNumbers(cpm, 2, 2);         
         n+=1;         
         
         gpio_put(Y_GP,0); // for sending UART data, RX indication pin must be pulled low when in sleep mode
         sleep_ms(10);  // specified are 5ms until module is able to send data via UART      
