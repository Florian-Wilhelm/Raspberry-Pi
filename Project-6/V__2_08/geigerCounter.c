...
void pwm_set_freq_duty(uint32_t f_clk_sys, uint slice_num, uint chan, uint32_t f, int d) {
    ...
    if (divider16 / 16 == 0)
      divider16 = 16;
    uint16_t wrap = clock * 16 / (divider16 * f) - 1;
    pwm_set_clkdiv_int_frac(slice_num, divider16/16, divider16 & 0xF);
    pwm_set_wrap(slice_num, wrap); // determining freq
    pwm_set_chan_level(slice_num, chan, wrap * d / 100); // determining duty cycle   
}
...
int main() {   
    ...
    uint f_clk_sys = (frequency_count_khz(CLOCKS_FC0_SRC_VALUE_CLK_SYS))*1000; // when unchanged, it is 125 MHz
