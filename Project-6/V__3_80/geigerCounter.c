/**
 * Created 10/04/2023 by Florian Wilhelm Dirnberger
 *
 ...
 * In addition, an OLED will show basic data (in order to save power, only very basic data will be displayed)
 * 
 ...
 * SW-version 3.8 includes a simple battery indicator (additional external resistors needed, not compatible with HW revision 3.x)
 * 
...
void show_digits(uint digit, uint a, uint b, uint c , uint d, uint s)
{
    ...
    struct render_area frame_area_s = {80, 87, 2, 2};
...
void battery_indication_show(uint16_t batteryValue)
{
    ...
    // note: ADC input resistance is not infinite! if voltage divider resistors are in the MOhm range (I've chosen 5 MOhm and 2.2 MOhm), a voltage drop can be expected
    // values for the voltage stated below are empirical
    
    if ((batteryValue * conversion_factor) > 2.40)
    ...
    else if ((batteryValue * conversion_factor) <= 1.9)
...
int main() {     
    ...
    */
    uint multiplier = 3;  // rather unclear what this parameter is really about, and whether it is useful in the given context (documentation left some question marks)      
    clock_configure(clk_sys,
                    CLOCKS_CLK_SYS_CTRL_SRC_VALUE_CLKSRC_CLK_SYS_AUX,
                    CLOCKS_CLK_SYS_CTRL_AUXSRC_VALUE_ROSC_CLKSRC,
                    multiplier * MHZ, // "Frequency of the input clock source"
                    multiplier * MHZ); // "Requested frequency"
                    
    uint32_t *ROSC = (uint32_t *)0x40060000; // ROSC_BASE
    
    *(ROSC + 0x10 / 4) = 0x00000aa0; // divider value (see RP2040 data sheet, chapter 2.17)
