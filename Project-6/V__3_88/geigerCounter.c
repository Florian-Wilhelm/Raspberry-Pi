/**
 * Created 20/04/2023 by Florian Wilhelm Dirnberger
 *
 ...
 * SW-version 3.8x includes a simple battery indicator (additional external resistors needed, not compatible with HW revision 3.x)
 * 
...
int main() {     
    ...
    */
    ...
    uint g = 0;
    uint k = 0;  
    float cpm = 0.0; // why a float? to generate a sound extrapolated value
    uint CPM = 0;
    float timeFrame = 2.0; // cpm calc method #1
    float pulsesFixed = 10.0;  // cpm calc method #2; why a float? to generate a sound extrapolated value
    ...
    for (;;) {
       ...
       if (gpio_get(CALC_METHODCONTROL_GP)){  // active by default       
         ...
         if (pulsesTimeframe == pulsesFixed) {
           ...
           cpm = (pulsesFixed/DeltaTime)*60.0;           
           pulsesTimeframe = 0;                            
