...
int main() {     
    ...
    */
    ...
    for (;;) {
       ...
       struct render_area tic_toc = {32, 47, 1, 1};       
       ...
       if ((absolutZeit/(timeFrame*n)) == 1) { 
         ...
         calc_render_area_buflen(&tic_toc);    
         render(clr_digit, &tic_toc);    
       ...
       if (absolutZeit/m == 60) {  
         ...
         calc_render_area_buflen(&tic_toc);  // indication for transition to the next minute
         render(ticking, &tic_toc);
