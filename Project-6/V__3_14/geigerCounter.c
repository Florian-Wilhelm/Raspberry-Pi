/**
 * Created 19/01/2023 by Florian Wilhelm Dirnberger
 *
...
void pwm_set_freq_duty(uint32_t f_clk_sys, uint slice_num, uint chan, uint32_t f, int d) {
    ...
    if (f < 2000)
       f = 2000;
    uint16_t wrap = clock/f; // 16bit - max value 65535, i.e. result must not exceed this value (min f is 1907 Hz for f_clk_sys 125 MHz)
